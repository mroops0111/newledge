...
const edges: GraphEdge[] = [
  ...
  edge('e5', 'supports', 'faster', 'offBoard'),
]
const onBoard = new Set(['faster', 'cheaper', 'rag', 'embedding', 'graphRag'])

describe('which relations the board draws', () => {
  it('draws what no arrangement could have said', () => {
    const drawn = drawnEdges(edges, onBoard, new Set())
    expect(drawn.map(one => one.id).sort()).toEqual(['e1'])
  })

  it('holds back what the arrangement already says, until a reader asks', () => {
    const asked = drawnEdges(edges, onBoard, new Set(['rag']))
    expect(asked.map(one => one.id).sort()).toEqual(['e1', 'e2', 'e3'])
  })
  ...
  it('spells out the verb only for the relation a reader asked about', () => {
    ...
    expect(asked.find(one => one.id === 'e1')?.label).toBeUndefined()
  })

  it('gathers what a concept is spoken about by, once a reader asks', () => {
    const asked = drawnEdges(edges, onBoard, new Set(['graphRag']))
    expect(asked.some(one => one.id === 'e4')).toBe(true)
  })

  it('routes classification as a diagram and argument as a curve', () => {
    const asked = drawnEdges(edges, onBoard, new Set(['rag']))
    expect(asked.find(one => one.id === 'e2')?.type).toBe('smoothstep')
    expect(asked.find(one => one.id === 'e1')?.type).toBe('default')
  })
})
