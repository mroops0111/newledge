...
(function () {
  ...
  function narrow(to) {
    questions.forEach(function (question) {
      var level = question.getAttribute('data-level')
      question.classList.toggle('away', to !== 'all' && level !== to)
    })
  }
  ...
  if (showing.length > 1) {
    ...
    all.forEach(function (one, at) {
      var chip = make('button', 'v-chip', one.label)
      chip.type = 'button'
      if (at === 0) chip.classList.add('on')
      chip.addEventListener('click', function () {
        var others = [].slice.call(chips.children)
  ...
  // One chapter is a page, not a sequence, so it is left as it arrived.
  ...
  function go(next) {
    at = Math.max(0, Math.min(chapters.length - 1, next))
    chapters.forEach(function (chapter, index) {
      chapter.classList.toggle('away', index !== at)
    })
    var walked = [].slice.call(dots.children)
    walked.forEach(function (dot, index) {
      dot.classList.toggle('on', index === at)
      dot.classList.toggle('behind', index < at)
    })
    where.textContent = (at + 1) + ' of ' + chapters.length
    back.disabled = at === 0
