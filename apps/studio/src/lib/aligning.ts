...
export function align(moving: Box, others: readonly Box[], tolerance = TOLERANCE): Alignment {
  ...
  return {
    at,
    guides: [
      ...(across === undefined ? [] : [guide(settled, across.other, 'x', across.line)]),
      ...(down === undefined ? [] : [guide(settled, down.other, 'y', down.line)]),
    ],
  }
}
    ...
    for (const theirs of spans(other, axis)) {
      for (const ours of mine) {
        ...
        if (gap >= closest)
          continue
        closest = gap
        best = { shift: theirs - ours, line: theirs, other }
      }
    }
  }
  return best
...
function guide(moving: Box, other: Box, axis: 'x' | 'y', at: number): Guide {
