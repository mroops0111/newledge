 ...
 * Each row is put in its best order given where everything else stands, so a
 * sweep can only shorten what the last one left, and the run stops early once
 * a whole sweep changes nothing. The count is only a backstop.
 */
const SWEEPS = 8

/**
 * How many in a row can have every order tried.
 * Seven is 5040 orders, which is nothing to measure. A board is not expected
 * to hold rows longer than that, and one that does is ordered by the estimate
 * instead rather than making the reader wait.
 */
 ...
 *
 * A packing places things by how well they fill a space, which says nothing
 * about what they are about, so two that relate can end up at opposite ends.
 * Where something sits in its row is free, in the way the order of siblings in
 * a family is free, and it is spent the same way, on putting what relates near
 * what it relates to.
 *
 * One row at a time is put in its best order given where every other row
 * currently stands, and the rows are gone over until none of them wants to
 * change. This is coordinate descent, so every step shortens the total and the
 * run has to end. The rows are gone over in alternating directions, so a
 * change made at one end of the board has a pass in which it can reach the
 * other. Nothing is tuned and nothing is random, so a board opens the same way
 * every time.
 *
 * A row keeps the ground it stood on. Its members are dealt back out between
 * the same two ends, so the packing keeps its shape and only the order changes.
 */
...
/**
 * Put one row in the order that leaves its relations shortest, and say whether
 * that was the order it was already in.
 */
function shortest(row: readonly string[], edges: readonly LayoutEdge[], at: Map<string, Box>): boolean {
  ...
  // Ties keep the order the row came in, since a board should not shuffle
  // itself around for nothing, so the order it had is measured first.
 ...
 * The gap comes out the same between every pair, since the row has a length to
 * fill and no reason to prefer one member of it to another.
 */
